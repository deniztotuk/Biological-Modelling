...
Usage:
    ...
    python run_app.py --export-sample  # Headless sample JPEG export
"""
...
def export_sample():
    ...
    samples = [
        (
            "competition_coexistence_light.jpeg",
            LotkaVolterraCompetitionModel(),
            (25, 20),
            {
                "r1": 0.8,
        ...
        (
            "predator_prey_cycles_light.jpeg",
            LotkaVolterraPredatorPreyModel(),
            (30, 10),
            {
                "r": 0.9,
        ...
        (
            "rosenzweig_macarthur_dark.jpeg",
            RosenzweigMacArthurModel(),
            (40, 15),
            {
                "r": 1.2,
    ...
    for filename, model, init_state, params, theme in samples:
        ...
        title_left = (
            f"Time Dynamics: {model.name}\n"
            f"({theme.capitalize()} Theme)"
        )
        ax1.set_title(
            title_left,
            fontsize=10,
        ...
        ax2.scatter(
            ...
            color=colors["start"],
            s=70,
            label=f"Start ({res.n1[0]:.1f}, {res.n2[0]:.1f})",
        )
        ax2.scatter(
            [res.n1[-1]],
            [res.n2[-1]],
            ...
            marker="X",
            s=80,
            label=f"End ({res.n1[-1]:.1f}, {res.n2[-1]:.1f})",
        )
        title_right = (
            "Phase Space (Respective to Each Other)\n"
            f"({theme.capitalize()} Theme)"
        )
        ax2.set_title(
            title_right,
        ...
        print(f"Saved: {filepath} ({file_kb} KB)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Biological Modelling Desktop Application"
    )
    parser.add_argument(
        "--theme",
        choices=["light", "dark"],
        default="light",
        help="Initial theme (default: light)",
    )
    parser.add_argument(
        "--export-sample",
        action="store_true",
        help="Generate sample JPEG graphs without launching GUI",
    )
